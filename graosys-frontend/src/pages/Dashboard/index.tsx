    ...
    <Card>
      <CardContent className="pt-6">
        <div className="flex items-center justify-between">
          ...
          <div
            className={`flex h-12 w-12 items-center justify-center rounded-full ${variant === "success" ? "bg-green-100" : variant === "warning" ? "bg-gray-100" : "bg-primary/10"}`}
          >
            <Icon
              className={`h-6 w-6 ${variant === "success" ? "text-green-600" : variant === "warning" ? "text-gray-500" : "text-primary"}`}
            />
          </div>
        </div>
      </CardContent>
          ...
          <>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <KpiCard
                ...
                value={String(data?.kpis.totalContracts ?? 0)}
                icon={FileText}
              />
              <KpiCard
              ...
              />
              <KpiCard
                title="Comissões Recebidas no Ano"
                value={formatCurrency(data?.kpis.totalReceived ?? 0)}
                icon={DollarSign}
                variant="success"
              />
              <KpiCard
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart
                      data={[
                        {
                          name: "Recebido",
                          valor: data?.kpis.totalReceived ?? 0,
                        },
                        {
                          name: "A Receber",
                          valor: data?.kpis.totalPending ?? 0,
                        },
                      ]}
                    >
